...
class GameRoomConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        connected_users = await self.add_user_to_room(
            self.room_name, self.scope["user"].nickname
        )
        ...
        await self.channel_layer.group_send(
            ...
            {"type": "join", "data": {"username": self.scope["user"].nickname}},
        )
    ...
    async def receive(self, text_data):
        ...
        if action == "ready":
            await self.channel_layer.group_send(
                self.room_group_name,
                {"type": "ready", "player": self.scope["user"].nickname},
            )
        ...
        if action == "game_start":
            ...
            player1 = (
                ...
                if self.player.piece_color == "white"
                    ...
                    name=connected_users[0]
                    if connected_users[0] != self.player.name
                    else connected_users[1],
                    piece_color="white",
                ).name
            )
            player2 = (
        ...
        if action == "show_players":
            ...
            await self.channel_layer.group_send(
                self.room_group_name,
                {"type": "show_players", "players": connected_users, "host": self.host},
            )

        elif action == "show_state":
            await self.channel_layer.group_send(
                self.room_group_name, {"type": "show_state"}
            )

        elif action == "make_move":
            player = self.player.name
            start_pos = (
                text_data_json["data"]["start_pos"][0],
                text_data_json["data"]["start_pos"][1],
            )
            end_pos = (
                text_data_json["data"]["end_pos"][0],
                text_data_json["data"]["end_pos"][1],
            )
            ...
            await self.channel_layer.group_send(
                self.room_group_name, {"type": "surrender"}
            )

    async def ready(self, event):
        await self.send(
            text_data=json.dumps({"type": "ready", "data": {"player": event["player"]}})
        )
    ...
    async def show_players(self, event):
        await self.send(
            text_data=json.dumps(
                {
                    "type": "show_players",
                    "data": {"players": event["players"], "host": event["host"]},
                }
            )
        )
    ...
    async def make_move(self, event):
        ...
        move_info = await self.game.make_move(
            player, (start_pos[0], start_pos[1]), (end_pos[0], end_pos[1])
        )
    ...
    async def show_state(self, event):
        state = self.game.show_state()
        await self.send(
            text_data=json.dumps({"type": "show_state", "data": {"state": state}})
        )
